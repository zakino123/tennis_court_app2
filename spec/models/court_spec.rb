...
RSpec.describe Court, type: :model do
  ...
  it '住所が重複している場合は無効' do
    FactoryBot.create(:court, address: 'test')
    other_court = FactoryBot.build(:court, address: 'test')
    other_court.valid?
    expect(other_court.errors[:address]).to include(I18n.t('errors.messages.taken'))
  end
